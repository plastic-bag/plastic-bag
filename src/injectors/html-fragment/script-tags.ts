...
export class ScriptTagHandler {
    ...
    private async processNextScript() {
        if (this.scripts.length > 0) {
            ...
            await runInNextTick(async () => this.processScriptTag(script, cloneScript));
        }
    }

    private async processScriptTag(script: HTMLScriptElement, cloneScript: HTMLScriptElement) {
    ...
    private async handleAsyncScriptLoad(script: HTMLScriptElement) {
        return new Promise<void>((resolve, reject) => {
            ...
            script.onerror = script.onabort = () => reject(new Error('Can not load script'));
        });
    }

    private copyScriptContent(script: HTMLScriptElement, cloneScript: HTMLScriptElement) {
